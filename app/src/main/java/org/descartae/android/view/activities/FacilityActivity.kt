...
import kotlinx.android.synthetic.main.content_facility.*
    ...
    fun renderFacility(facility: FacilityQuery.Facility) {
        ...
        for (openHour in facility.openHours()) {
            if (openHour.dayOfWeek().ordinal + 1 == Calendar.getInstance().get(Calendar.DAY_OF_WEEK)) {
                time = getString(R.string.time, "Hoje", getString(R.string.time_desc, openHour.startTime(), openHour.endTime()))
                text_time.text = time
                break
            }
        }
