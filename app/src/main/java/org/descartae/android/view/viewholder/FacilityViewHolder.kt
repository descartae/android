        ...
        mItem?.let {
            ...
            for (typesOfWaste in it.typesOfWaste()) {
                ...
                if (itemView.type_waste.childCount == 4) {
                    Picasso.get().load(R.drawable.ic_waste_more).resizeDimen(R.dimen.type_waste_height, R.dimen.type_waste_height).into(ii)
                    itemView.type_waste.addView(ii)
                    break
                } else {
                    Picasso.get().load(typesOfWaste.icons().androidMediumURL())
